...
public class ParticleSystem {
    ...
    private ParticleSystem(Activity a, Resources res, int maxParticles, long timeToLive, int
			parentResId) {
		mRandom = new Random();
        ...
        // either res or activity is valid, else this is an error
        res = res == null ? a.getResources() : res;
	...
	public ParticleSystem(Activity a, int maxParticles, int drawableRedId, long timeToLive) {
		this(a, null, maxParticles, a.getResources().getDrawable(drawableRedId), timeToLive,
    ...
    public ParticleSystem(Resources res, int maxParticles, int drawableRedId, long timeToLive) {
        this(null, res, maxParticles, res.getDrawable(drawableRedId), timeToLive,
                android.R.id.content);
    }
    ...
    public ParticleSystem(Activity a, int maxParticles, Drawable drawable, long timeToLive) {
        this(a, null, maxParticles, drawable, timeToLive, android.R.id.content);
    }
     ...
     * @param timeToLive The time to live for the particles
     */
    public ParticleSystem(Resources res, int maxParticles, Drawable drawable, long timeToLive) {
        this(null, res, maxParticles, drawable, timeToLive, android.R.id.content);
    }

	/**
	 * Utility constructor that receives a Drawable. Either a valid activity or a valid resources
     * object is required.
	 * 
	 * @param a The parent activity
     * @param res A resources object associated with the relevant context.
     ...
     * @param parentViewId The view Id for the parent of the particle system
	 */
	public ParticleSystem(Activity a, Resources res, int maxParticles, Drawable drawable, long
            timeToLive, int parentViewId) {
		this(a, res, maxParticles, timeToLive, parentViewId);
...
//		}
	...
	public ParticleSystem(Activity a, int maxParticles, Bitmap bitmap, long timeToLive, int parentViewId) {
		this(a, null, maxParticles, timeToLive, parentViewId);
		for (int i=0; i<mMaxParticles; i++) {
			mParticles.add (new Particle (bitmap));
		}
	}
	...
	private void startEmiting(int particlesPerSecond) {
        ...
        mDrawingView.setMimiDimensionView(mParentView);
	...
	private void configureEmiter(int emitterX, int emitterY) {
		...
		// if present
		mEmiterXMin = mIgnorePositionInParent ? emitterX : emitterX - mParentLocation[0];
